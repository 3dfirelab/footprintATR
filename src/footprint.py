...
##################################################
def imu2ext_param(lat,lon,alt,roll,pitch,yaw, correction_xyz,correction_opk, crs_code, transformer, transformer_inv):
...
#################################################
def orthro(tiffile, time, lat,lon,alt,roll,pitch,yaw, params ):
   
    correction_xyz = params['correction_xyz']
    correction_opk = params['correction_opk']
    wkdir          = params['wkdir']
    crs_code       = params['crs_code']
    transformer_inv= params['transformer_inv']
    transformer    = params['transformer']
    transformer    = params['transformer']
    intparamFile   = params['intparamFile'] 
    demFile        = params['demFile']

    ext_param = imu2ext_param(lat,lon,alt,roll,pitch,yaw, correction_xyz,correction_opk, crs_code, transformer, transformer_inv)
    str_tag = 'mm'
...
def tiff_bounds_to_gdf(tiff_path, time):
    ...
    '''
    # Open TIFF with rioxarray
    da = rioxarray.open_rasterio(tiff_path, masked=True).squeeze().isel(band=0)  # remove extra dims
    ...
    gdf = gdf[gdf.area>1.e3] # to remove the potential small polygon generated in the mask
